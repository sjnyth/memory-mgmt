...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
   ...
   */
   ...
   if (!curr_node->next) {
      if (node_block->blk->start > curr_node->blk->start) {
        node_block->next = NULL;
        curr_node->next = node_block;
      } else {
        ...
        if (node_block->blk->start <= curr_node->blk->start) {
            ...
            while(curr_node && node_block->blk->start > curr_node->blk->start) {
                prev = curr_node;
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
   ...
   */
   ...
   int node_block_size = node_block->blk->end - node_block->blk->start;
   int curr_block_size = curr_node->blk->end - curr_node->blk->start + 1;
   node_t* prev = curr_node;
   if (!curr_node->next) {
      if (node_block_size > curr_block_size) {
        node_block->next = NULL;
        curr_node->next = node_block;
      } else {
        node_block->next = curr_node;
        ...
        if (node_block_size <= curr_block_size) {
            ...
            while(curr_node && node_block_size > curr_block_size) {
                prev = curr_node;
                curr_node = curr_node->next;
                if (curr_node) {
                    curr_block_size = curr_node->blk->end - curr_node->blk->start;
                }
            }
            prev->next = node_block;
            node_block->next = curr_node;
...
bool list_is_in_by_pid(list_t *l, int pid){ 
  ...
  */
  node_t* curr_node = l->head;
  while (curr_node) {
    if (comparePid(pid, curr_node->blk)) {
      return true;
    }
    curr_node = curr_node->next;
  }
